...
app.post('/api/files', upload.single('file'), async (req, res) => {
  ...
  if (file.mimetype !== 'text/csv') {
    return res.status(500).json({ message: 'File must ve CSV' })
  }

  let json: Array<Record<string, string>> = []
  try {
    // 4. Transform File (Buffer) to string
    const rawCsv = file.buffer.toString('utf-8') // ó Buffer.from(file.buffer).toString('utf-8')
    // console.log(rawCsv);
  ...
  userData = json
  // 7. Return with the message and the JSON
  return res
    .status(200)
    .json({ message: 'El archivo se cargo correctamente', data: userData })
})
